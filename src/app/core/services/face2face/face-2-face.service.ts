...
export class Face2FaceService {
    ...
    async checkIsProAndGoToFace2Face(opponentUniqueIndex: number, presentingElement: HTMLElement): Promise<void> {
        const isPro = await this.isPro.isPro$(presentingElement).toPromise();
        if (isPro) {
